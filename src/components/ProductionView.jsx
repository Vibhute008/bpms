...
const aggregateByDate = (entries) => {
  ...
  entries.forEach(entry => {
    ...
    aggregated[entry.date].totalQuantity += entry.quantity;

    if (entry.projectName) {
      if (!aggregated[entry.date].projects[entry.projectName]) {
        aggregated[entry.date].projects[entry.projectName] = 0;
      }
      aggregated[entry.date].projects[entry.projectName] += entry.quantity;
    }

    // Factory-specific tracking
    if (entry.factory === 'Mahape') {
      aggregated[entry.date].mahape += entry.quantity;
    } else if (entry.factory === 'Taloja') {
      aggregated[entry.date].taloja += entry.quantity;
    } else if (!entry.factory) {
      // Backward compatibility: infer factory from project name or photos
      if (entry.projectName && entry.projectName.includes('Mahape')) {
        aggregated[entry.date].mahape += entry.quantity;
      } else if (entry.projectName && entry.projectName.includes('Taloja')) {
        aggregated[entry.date].taloja += entry.quantity;
      } else if (entry.photos && entry.photos.length > 0) {
        if (entry.photos[0].url && entry.photos[0].url.includes('mahape')) {
          aggregated[entry.date].mahape += entry.quantity;
        } else if (entry.photos[0].url && entry.photos[0].url.includes('taloja')) {
          aggregated[entry.date].taloja += entry.quantity;
        }
      }
    } else if (entry.factory === 'N/A') {
      // Entries with unknown factory - don't count toward specific factories
      // They won't contribute to factory-specific charts
    }
  });
...
// Function to aggregate data by project
const aggregateByProject = (entries) => {
  const aggregated = {};

  entries.forEach(entry => {
    const projectName = entry.projectName || 'Unknown Project';

    if (!aggregated[projectName]) {
      aggregated[projectName] = {
        name: projectName,
        totalQuantity: 0,
        entries: 0
      };
    ...
    aggregated[projectName].totalQuantity += entry.quantity;
    aggregated[projectName].entries += 1;
  });
...
const prepareChartData = (entries) => {
  const dailyData = aggregateByDate(entries);
  const projectData = aggregateByProject(entries);

  // Prepare factory distribution data
  const factoryDistribution = [
    { name: 'Mahape', value: dailyData.reduce((sum, day) => sum + day.mahape, 0) },
    { name: 'Taloja', value: dailyData.reduce((sum, day) => sum + day.taloja, 0) }
  ];
...
export default function ProductionView({ user, onLogout }) {
  ...
  useEffect(() => {
    ...
    // Enhance loaded entries with factory information for backward compatibility
    const enhancedEntries = loadedEntries.map(entry => {
      if (!entry.factory) {
        // Infer factory from project name or photos
        if (entry.projectName && entry.projectName.includes('Mahape')) {
          return { ...entry, factory: 'Mahape' };
        } else if (entry.projectName && entry.projectName.includes('Taloja')) {
          return { ...entry, factory: 'Taloja' };
        } else if (entry.photos && entry.photos.length > 0) {
          if (entry.photos[0].url && entry.photos[0].url.includes('mahape')) {
            return { ...entry, factory: 'Mahape' };
          } else if (entry.photos[0].url && entry.photos[0].url.includes('taloja')) {
        ...
        // Try to get factory from project information
        const project = loadedProjects.find(p => p.id === entry.projectId);
        if (project && project.factory) {
          return { ...entry, factory: project.factory };
        }
        // If we can't determine factory, default to N/A
        return { ...entry, factory: 'N/A' };
      }
      return entry;
    });
    
    setEntries(enhancedEntries);
    setFilteredEntries(enhancedEntries);
    setProjects(loadedProjects);
    setChartData(prepareChartData(enhancedEntries));
  }, [user]);
  ...
  const handleAddEntry = async (e) => {
    ...
    const entryFactory = user.role === 'OPERATOR' ? user.factory : project.factory;
    ...
    const saveFactory = user.role === 'OPERATOR' ? user.factory : project.factory;
    const factoryEntries = loadProductionData(saveFactory);
    factoryEntries.unshift(entry);
    saveProductionData(saveFactory, factoryEntries);

    // Update state
    if (user.role === 'OPERATOR' && user.factory === saveFactory) {
      setEntries(prev => [entry, ...prev]);
      setFilteredEntries(prev => [entry, ...prev]);
      setChartData(prepareChartData([entry, ...entries]));
  ...
  const handleUpdateEntry = async (e) => {
    ...
    // Save to localStorage
    const saveFactory = user.role === 'OPERATOR' ? user.factory : project.factory;
    const factoryEntries = loadProductionData(saveFactory).map(entry =>
    ...
    setChartData(prepareChartData(factoryEntries));
                ...
                filteredEntries.map((entry) => (
                  <tr key={entry.id} className="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${entry.factory === 'Mahape' || (!entry.factory && entry.projectName && entry.projectName.includes('Mahape')) ? 'bg-green-100 text-green-800' : entry.factory === 'Taloja' || (!entry.factory && entry.projectName && entry.projectName.includes('Taloja')) ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-800'}`}>
                        {entry.factory || (entry.projectName && entry.projectName.includes('Mahape') ? 'Mahape' : entry.projectName && entry.projectName.includes('Taloja') ? 'Taloja' : 'N/A')}
                      </span>
                    </td>
                ...
                <div className="p-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div className="bg-gray-50 rounded-xl p-5 border border-gray-200">
                      ...
                      <p className="text-lg font-semibold text-gray-900">{entry.date}</p>
                    </div>
                    <div className="bg-gray-50 rounded-xl p-5 border border-gray-200">
                      <label className="block text-sm font-medium text-gray-700 mb-2">Project</label>
                      <p className="text-lg font-semibold text-gray-900">{entry.projectName}</p>
                    </div>
                    <div className="bg-gray-50 rounded-xl p-5 border border-gray-200">
                      <label className="block text-sm font-medium text-gray-700 mb-2">Quantity</label>
                      <p className="text-lg font-semibold text-gray-900">{formatNumber(entry.quantity)} units</p>
                    </div>
                    <div className="bg-gray-50 rounded-xl p-5 border border-gray-200">
                      <label className="block text-sm font-medium text-gray-700 mb-2">Factory</label>
                      <p className="mt-1">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${entry.factory === 'Mahape' || (!entry.factory && entry.projectName && entry.projectName.includes('Mahape')) ? 'bg-green-100 text-green-800' : entry.factory === 'Taloja' || (!entry.factory && entry.projectName && entry.projectName.includes('Taloja')) ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-800'}`}>
                          {entry.factory || (entry.projectName && entry.projectName.includes('Mahape') ? 'Mahape' : entry.projectName && entry.projectName.includes('Taloja') ? 'Taloja' : 'N/A')}
                        </span>
                      </p>
                    </div>
                  ...
                  <div className="mt-8">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-indigo-100 text-indigo-800">
                        {entry.photos ? entry.photos.length : 0} photos
                      </span>
                    </div>
